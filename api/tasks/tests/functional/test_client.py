...
import pytest
from tasks.factories import TaskFactory, TaskTypeFactory
...
class TestTaskApi:
    ...
    def test_update_task(self, client):
        ...
        response = client.patch(f'/api/tasks/{task.task_id}/', json.dumps({'status': Task.Status.COMPLETED.value}), content_type='application/json')
        assert response.status_code == 200
        data = response.json()
        assert data['task_id'] == str(task.pk)
    ...
    def test_create(self, client):
        ...
        response = client.post(f'/api/tasks/', json.dumps({'name': task_type.name, 'processing_time': expected_processing_time}), content_type='application/json')
